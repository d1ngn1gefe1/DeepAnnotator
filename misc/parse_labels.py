...
def main(params):
    playlists = [d for d in os.listdir(params['video_dir']) if d.find('10.') != -1]
    for p in playlists:
        input_file = os.path.join(params['data_dir'], p, params['input_file']) 
        output_file = os.path.join(params['data_dir'], p, 'records.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-video_dir', default='../public/static/video')
    parser.add_argument('-input_file', default='detections-22-15.txt',
                        help='Input filename')
